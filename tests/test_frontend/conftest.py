...
def _run_task_cleanup(page, cleanup_info):
    ...
    with allure.step(f'Cleanup: удаление задач с таймстемпом {ts}'):
        ...
        allure.attach(
            f'Удалено карточек: {deleted}', name='cleanup result', attachment_type=allure.attachment_type.TEXT
        )
...
def cleanup_cards_by_pattern(page, pattern: str):
    ...
    with allure.step(f"Cleanup: удаление карточек по паттерну '{pattern}'"):
        ...
        if deleted:
            allure.attach(
                f'Удалено карточек: {deleted}', name=f"cleanup '{pattern}'", attachment_type=allure.attachment_type.TEXT
            )
    ...
    def _assert(screenshot: bytes, name: str, threshold: float = 1.0):
        ...
        if diff_pct > threshold:
            if diff_coords:
                ...
                allure.attach(
                    buf.getvalue(), name=f'diff ({diff_pct:.2f}%)', attachment_type=allure.attachment_type.PNG
                )
