...
def test_01_add_task_subtasks(page: Page, soft_step, sidebar):
    ...
    with allure.step('Проверка: 0 subtasks'):
        soft_step(
            '0 subtasks', lambda: expect(sidebar.get_by_role('heading', name='0 subtasks')).to_be_visible(timeout=5000)
        )
    ...
    _check_counter(
        page,
        '1 subtask',
        lambda: expect(sidebar.get_by_role('heading', name=re.compile(r'\b1 subtask\b'))).to_be_visible(timeout=10000),
    )

    _check_counter(
        page, '0 completed of 1', lambda: expect(sidebar.get_by_text('0 completed of 1')).to_be_visible(timeout=10000)
    )
    ...
    _check_counter(
        page,
        '2 subtasks',
        lambda: expect(sidebar.get_by_role('heading', name='2 subtasks')).to_be_visible(timeout=10000),
    )

    _check_counter(
        page, '0 completed of 2', lambda: expect(sidebar.get_by_text('0 completed of 2')).to_be_visible(timeout=10000)
    )
...
def test_02_complete_task_subtask(page: Page, soft_step):
    ...
    with allure.step(f'Снятие завершения: {_SUBTASK_NAME_2}'):
        soft_step(
            'Снятие завершения подзадачи 2', lambda: toggle_subtask_complete(page, _SUBTASK_NAME_2, full_page=True)
        )
...
def test_03_delete_task_subtask(page: Page, soft_step, sidebar):
    ...
    with allure.step('Проверка: 1 subtask'):
        soft_step(
            '1 subtask',
            lambda: expect(sidebar.get_by_role('heading', name=re.compile(r'\b1 subtask\b'))).to_be_visible(
                timeout=10000
            ),
        )
    ...
    with allure.step('Проверка: 0 subtasks'):
        soft_step(
            '0 subtasks', lambda: expect(sidebar.get_by_role('heading', name='0 subtasks')).to_be_visible(timeout=10000)
        )
