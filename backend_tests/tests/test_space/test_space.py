import allure
from backend_tests.data.endpoints.Space.space_endpoints import create_space_endpoint, get_space_endpoint, get_spaces_endpoint, edit_space_endpoint, remove_space_endpoint
...
def test_remove_space_success(owner_client):
    ...
    with allure.step('Ensure space is no longer in list'):
        ...
        assert all((s['_id'] != space_id for s in list_response.json()['payload']['spaces'])), f'Removed space {space_id} всё ещё найден в GetSpaces'
