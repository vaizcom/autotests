...
def first_custom_field_id(owner_client, main_space, main_board):
    """_id первого custom field на main_board."""
    resp = owner_client.post(**get_board_endpoint(main_board, main_space))
    assert resp.status_code == 200, resp.text
    return resp.json()["payload"]["board"]["customFields"][0]["_id"]


@allure.parent_suite("Board Service")
@allure.suite("Access board")
@allure.sub_suite("ReorderBoardCustomField")
...
@allure.title("ReorderBoardCustomField: {client_fixture} → 200")
def test_reorder_field_allowed(
    request, client_fixture, main_space, main_board, first_custom_field_id,
):
    """Пользователи с правами управления бордой могут менять порядок custom fields."""
    client = request.getfixturevalue(client_fixture)

    with allure.step("Перемещаем поле на ту же позицию (без изменений)"):
        resp = client.post(**reorder_board_custom_field_endpoint(
            board_id=main_board, space_id=main_space,
            field_id=first_custom_field_id, from_index=0, to_index=0,
        ))
...
@allure.title("ReorderBoardCustomField: {client_fixture} → 403 AccessDenied")
def test_reorder_field_denied(
    request, client_fixture, main_space, main_board, first_custom_field_id,
):
    """Пользователи без прав управления бордой не могут менять порядок custom fields."""
    client = request.getfixturevalue(client_fixture)

    with allure.step("Пытаемся переместить поле"):
        resp = client.post(**reorder_board_custom_field_endpoint(
            board_id=main_board, space_id=main_space,
            field_id=first_custom_field_id, from_index=0, to_index=0,
        ))
