...
@allure.parent_suite("Auth Service")
@allure.suite("Registration Validation")
def test_register_missing_password_field(base_url=API_URL):
    """
    Тест регистрации без передачи поля password в JSON (обязательное поле).
    """
    allure.dynamic.title("Register: Отказ при отсутствии поля password в запросе")

    timestamp = int(time.time())
    user_email = f"missing_pass_{timestamp}@gmail.com"
    user_name = f"user_{timestamp}"

    with allure.step("Подготовка данных и удаление поля password"):
        endpoint_data = register_endpoint(
            email=user_email,
            password="temporary_value", # Значение будет удалено
            full_name=user_name,
            terms_accepted=True
        )

        # Удаляем ключ password из тела запроса
        if 'password' in endpoint_data['json']:
            del endpoint_data['json']['password']
    ...
    with allure.step("Проверка, что ошибка относится к password"):
        resp_json = resp.json()
        assert "password" in str(resp_json).lower(), "В ошибке не упоминается пропущенное поле password"