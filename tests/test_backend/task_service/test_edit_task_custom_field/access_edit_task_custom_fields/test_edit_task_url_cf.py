...
def test_edit_task_url_custom_field_roles(
        request,
        main_space,
        owner_client,
        client_fixture_name,
    ...
    allure.dynamic.title(
        f"URL {client_fixture_name} : URL Custom Fields. Проверка доступа к редактированию поля для роли {client_fixture_name}")

    """
    URL Custom Fields. Проверка доступа к редактированию поля URL для разных ролей.
    """
    client = request.getfixturevalue(client_fixture_name)
    target_task_id = "696a1a04c7fd1dbba471efc2"
    url_field_id = "696e02e92452157dfd7e2631"
    new_value = "https://www.google.com"

    # Pre-condition: Очищаем поле через owner для чистоты эксперимента
    if expected_status_code == 200:
        _update_custom_field(owner_client, main_space, target_task_id, url_field_id, "")

    with (allure.step(f"Action: Попытка изменения поля URL под ролью {client_fixture_name}")):
        resp_edit = _update_custom_field(client, main_space, target_task_id, url_field_id, new_value)
    ...
    if expected_status_code == 200:
        with allure.step("Verification: Если доступ разрешен, проверяем, что значение действительно обновилось"):
            ...
            updated_field = next((cf for cf in task.get("customFields", []) if cf["id"] == url_field_id), None)
