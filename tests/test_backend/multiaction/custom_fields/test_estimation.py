...
@allure.sub_suite("Estimation")
@allure.title("Estimation: установить длительность")
def test_estimation_set_value(
    owner_client, main_space, make_task_in_main, temp_board_in_main, estimation_field_id,
):
    new_value = "P1W2DT3H"

    with allure.step("Создаём задачу"):
        task = make_task_in_main({"name": "cf-est-set", "board": temp_board_in_main})
        task_id = task["_id"]

    with allure.step("Устанавливаем Estimation через multiaction"):
        resp = owner_client.post(**multiple_edit_tasks_custom_field_endpoint(
            space_id=main_space, tasks_ids=[task_id],
            custom_field_id=estimation_field_id, value=new_value,
        ))
    ...
    with allure.step("Проверяем через GetTask"):
        ...
        assert _get_cf_value(r.json()["payload"]["task"], estimation_field_id) == new_value


@allure.parent_suite("Multiaction")
@allure.suite("Custom Fields")
@allure.sub_suite("Estimation")
@allure.title("Estimation: значение совпадает → skipped")
def test_estimation_same_value_skipped(
    owner_client, main_space, make_task_in_main, temp_board_in_main, estimation_field_id,
):
    value = "PT5H30M"

    with allure.step("Создаём задачу и устанавливаем Estimation"):
        task = make_task_in_main({"name": "cf-est-skip", "board": temp_board_in_main})
        task_id = task["_id"]
        r = owner_client.post(**edit_task_custom_field_endpoint(
            space_id=main_space, task_id=task_id, field_id=estimation_field_id, value=value,
        ))
        assert r.status_code == 200, r.text
    ...
    with allure.step("Применяем multiaction с тем же значением"):
        resp = owner_client.post(**multiple_edit_tasks_custom_field_endpoint(
            space_id=main_space, tasks_ids=[task_id],
            custom_field_id=estimation_field_id, value=value,
        ))
