...
def test_create_and_fill_task(page: Page, cleanup_board, assert_snapshot):
    ...
    # === ПОЛЯ (soft assertions — при падении одного поля тест продолжается, но всё равно упадёт) ===
    ...
    with allure.step("Майлстоун"):
        def add_milestone():
            ...
            page.get_by_role("textbox", name="Type name").fill("м1")
    ...
    with allure.step("Блокер и блокинг"):
        def add_blockers():
            page.get_by_role("textbox", name="Add blocker").fill("1")
            page.get_by_role("button", name=re.compile(r"Blockers.*Create task")).get_by_role("button").click()
            page.get_by_role("textbox", name="Add blocking").fill("2")
    ...
    # === СКРИНШОТ ===
    with allure.step("Сравнение скриншота задачи"):
        ...
        assert_snapshot(screenshot, name="task_fields.png", threshold=1.5)
