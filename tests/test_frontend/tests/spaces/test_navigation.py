...
def test_navigation(page: Page, soft_step):
    ...
    with allure.step(f'Открытие {_SPACE_NAME}'):
        soft_step(
            ...
            lambda: (
                ...
                or page.get_by_test_id(SpaceSelector.space(settings.AUTOTEST_SPACE_ID)).click()
            ),
        )
    ...
    with allure.step(f'Проверка видимости {_PROJECT_NAME}'):
        soft_step(
            'Проект виден',
            lambda: expect(page.locator('span').filter(has_text=_PROJECT_NAME)).to_be_visible(timeout=10000),
        )

    with allure.step(f'Открытие {_PROJECT_NAME}'):
        soft_step(
            'Открытие проекта', lambda: page.locator('span').filter(has_text=_PROJECT_NAME).get_by_role('link').click()
        )
