...
from test_backend.data.endpoints.Document.document_endpoints import create_document_endpoint, \
    duplicate_document_endpoint, archive_document_endpoint
...
def test_duplicate_document_access_by_roles(request, main_space, client_fixture, expected_status):
    ...
    create_resp = api_client.post(
        **create_document_endpoint(kind='Space', kind_id=main_space, space_id=main_space, title=f"{current_date} {role} Doc For Duplication")
    )
    if create_resp.status_code != 200:
        with allure.step(f"Не удалось создать документ, статус {create_resp.status_code} — пропуск дублирования"):
            assert expected_status == 403, f"Невозможно протестировать дублирование без документа (статус создания {create_resp.status_code})"
        return

    doc_id = create_resp.json()["payload"]["document"]["_id"]

    with allure.step(f"{role} пытается продублировать документ {doc_id}"):
    ...
    with allure.step(f"Архивация исходного документа {doc_id}"):
