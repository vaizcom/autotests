import allure
import pytest
from tests.backend.data.endpoints.Board.board_endpoints import (
...
from tests.backend.data.endpoints.Board.custom_field_types import CustomFieldType
from tests.backend.utils.generators import generate_board_name, generate_custom_field_title, generate_object_id
from tests.backend.data.endpoints.Board.constants import (
...
def test_create_board_group_description_validation(owner_client, temp_board, temp_space, description, expected_status):
    ...
    with allure.step(
        f"Проверка, что API вернул {expected_status} при description длиной {len(description) if description else 'None'}"
    ):
        assert response.status_code == expected_status
...
@allure.title(
    'Ошибка при создании борды с заголовком custom field длиннее 25 символов (Баг: до 50 символов - APP-2763 )'
)
