...
def test_archive_unarchive_project(request, client_fixture, expected_status, owner_client, main_space):
    allure.dynamic.title(f'Тест архивации и разархивации project: клиент={client_fixture}, ожидаемый статус={expected_status}')
    project_id = '693fe53af55da7e1e4ca99f7'
    ...
    with allure.step(f'Шаг 1. Подготовка к тесту: Проверка и сброс состояния проекта (если архивирован) для {client_fixture}'):
        ...
        current_get_response = owner_client.post(**get_project_endpoint(project_id=project_id, space_id=main_space))
        ...
        if project_needs_unarchive:
            unarchive_initially_response = owner_client.post(**unarchive_project_endpoint(project_id=project_id, space_id=main_space))
        ...
        final_initial_check_response = owner_client.post(**get_project_endpoint(project_id=project_id, space_id=main_space))
    ...
    with allure.step(f'Шаг 2: Попытка архивировать проект с ролью {client_fixture}, ожидаемый статус={expected_status}'):
        archive_response = client.post(**archive_project_endpoint(project_id=project_id, space_id=main_space))
        ...
        if expected_status == 200:
            ...
            get_archived_project_response = client.post(**get_project_endpoint(project_id=project_id, space_id=main_space))
            ...
            get_project_after_failed_archive = owner_client.post(**get_project_endpoint(project_id=project_id, space_id=main_space))
    ...
    with allure.step(
        ...
        if expected_status == 200:  # Если клиент смог архивировать (owner/manager), он должен смочь и разархивировать
            unarchive_response = client.post(**unarchive_project_endpoint(project_id=project_id, space_id=main_space))
            ...
            final_project_data_response = owner_client.post(
                **get_project_endpoint(project_id=project_id, space_id=main_space))
